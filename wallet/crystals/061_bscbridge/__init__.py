...
class BscbridgeHandler(CrystalHandler):
    ...
    def initialize(self):
        ...
        for script in scripts:
            footer += f'<script src= "/crystal/bscbridge/static/{script}"></script>\n'
    ...
    async def to_eth2(self, params=None):
        """
        /crystal/bscbridge/to_eth2?eth_address=0x&amount=10&txidhash=0x&auth=0x
        """
        mint = dict()
        ...
        data = {}
        self.bismuth_vars['extra']["footer"] += f'<script src= "/crystal/bscbridge/static/to_eth2.js"></script>\n'
    ...
    async def from_eth1(self, params=None):
        ...
        data = {}
        # self.bismuth_vars['extra']["footer"] += f'<script src= "/crystal/bscbridge/static/to_eth2.js"></script>\n'
        self.bismuth_vars['extra']["footer"] += f'<script src= "/crystal/{OPERATION_PREFIX}/static/from_eth1.js"></script>\n'
    ...
    async def swaps(self, params=None):
        ...
        """bis_data = self.bismuth.command("listopfromto",
                            [self.bismuth_vars["address"], ETH_BRIDGE_ADDRESS, f"{OPERATION_PREFIX}:send", 10, True, 0, 9e10]
                                        )
        """
        ...
        bismuth_params = [data["bis_address"], ETH_BRIDGE_ADDRESS,
                          f'{OPERATION_PREFIX}:send', '0.00000000', True, 0, 9e10]
        ...
        bismuth_params = [ETH_ORACLE_ADDRESS, data["bis_address"],
                          f'{OPERATION_PREFIX}:burn', '0.00000000', True, 0, 9e10]
        bis_data = self.bismuth.command(command, bismuth_params)
        ...
        bismuth_params = [ETH_BRIDGE_ADDRESS, data["bis_address"], f'{OPERATION_PREFIX}:signature', '0.00000000', True, 0, 9e10]
        bis_data = self.bismuth.command(command, bismuth_params)
        for tx in bis_data:
            try:
                ...
                pass
        # Once wBIS Minted
        bismuth_params = [ETH_ORACLE_ADDRESS, data["bis_address"], f'{OPERATION_PREFIX}:proxymint', '0.00000000', True, 0, 9e10]
        bis_data = self.bismuth.command(command, bismuth_params)
        ...
        for tx in bis_data:
            try:
                ...
                pass
        # Once BIS delivered
        bismuth_params = [ETH_BRIDGE_ADDRESS, data["bis_address"], f'{OPERATION_PREFIX}:deliver', '0.00000000', True, 0, 9e10]
        bis_data = self.bismuth.command(command, bismuth_params)
        # print("proxymint", bis_data)
        for tx in bis_data:
            try:
